...
function setup() {
  ...
  let button = createButton('toggle outlines');
  button.position(900, 100);

  // Call repaint() when the button is pressed.
  button.mousePressed(toggleOutlines);
  ...
  let thisinput = createInput("xyzzy");
  thisinput.parent(label);
  thisinput.input(() => {
    rebuildWorld(thisinput.value());
  });
...
function drawTile([world_x, world_y], [camera_x, camera_y]) {
  ...
  if (window.p3_drawTile) {
    window.p3_drawTile(world_x, world_y,timePassed,outlines);// -screen_x, screen_y,timepassed = timePassed);
